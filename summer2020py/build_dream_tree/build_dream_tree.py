...
def run_AffinityProp(row_or_col, data_df, args):
    if row_or_col == "col":
...
def loop_through_clusters(total_linkage_matrix, label_df, num_row, super_dendro_labels_index, data_df):
    ...
    #get all the values in the current sub cluster
        ...
        if sub_cluster.shape[0] != 1:
            ...
            total_linkage_matrix.loc[where.index[0], [where_col + 4]] = data_df.columns.get_loc(sub_cluster.index[0])    
...
def create_dendrogram_from_df(row_or_col, data_df, args):
    #calling above methods in some order that it creates the dendrogram
    AffinityProp = run_AffinityProp(row_or_col, data_df, args)
...
def prepare_gctoo_for_json(gctoo, new_data_df):
    sas_flipped_index = numpy.flip(list(new_data_df.index))
    sas_flipped_col = numpy.flip(list(new_data_df.columns))

    new_row_metadata = gctoo.row_metadata_df.loc[sas_flipped_index]
    new_col_metadata = gctoo.col_metadata_df.loc[sas_flipped_col]

    new_data_df = new_data_df.loc[sas_flipped_index]
    new_data_df = new_data_df.loc[:, sas_flipped_col]

    logger.debug("gctoo.row_metadata_df:{}".format(new_row_metadata))
    logger.debug("gctoo.col_metadata_df:{}".format(new_col_metadata))
    logger.debug("gctoo.data_df: {}".format(new_data_df))

    new_gctoo = cmapPy.pandasGEXpress.GCToo.GCToo(new_data_df, row_metadata_df=new_row_metadata, col_metadata_df=new_col_metadata, src=None, version=None, make_multiindex=False, logger_name='cmap_logger')
...
def create_json(template, output_path, gctoo, col_newick, row_newick):
    ...
    dataset["rowMetadataModel"]["vectors"].append(create_col_and_row_metadata_template(True, "rid", sas_flipped_index)) 
    dataset["columnMetadataModel"]["vectors"].append(create_col_and_row_metadata_template(True, "cid", sas_flipped_col))
...
def main(args):
    ...
    if args.row_or_col == "col":
        col_newick, new_data_df = create_dendrogram_from_df("col", gctoo.data_df, args)
        row_newick = None

    elif args.row_or_col == "row":
        row_newick, new_data_df = create_dendrogram_from_df("row", gctoo.data_df, args)
        col_newick = None

    elif args.row_or_col == "both":
        col_newick, new_data_df = create_dendrogram_from_df("col", gctoo.data_df, args)
        row_newick, new_data_df = create_dendrogram_from_df("row", new_data_df, args)

    
    new_gctoo = prepare_gctoo_for_json(gctoo, new_data_df)


    create_json(summer2020py.morpheus_heatmap_template, output_path, new_gctoo,  col_newick, row_newick)
