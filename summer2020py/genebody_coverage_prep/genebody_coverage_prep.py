...
def main(args):
    main_params(args.SAMPLES, args.SAMPLEFILE, args.output_dir, args.input_dir)


def main_params(samples, sample_file, output_dir, input_dir):
    Sample_list = None
    ...
    if(sample_file is None ): #checking to see if the command line used the file or if they just added a Sample
        ...
        Sample_list = samples #adding the sample given on the command line into the sample list
    else:
        logger.debug("using sample_file")
        logger.debug("sample_file:  {}".format(sample_file))
        Sample_list = load_sample(sample_file) #calling the method load sample to add all of the samples in the file to the sample list
    prepare_output_dir(output_dir) #preparing the output directory 
    for sample in Sample_list: #each sample in the sample list runs through the for loop
        prepare_sample_dir(sample, output_dir, input_dir) #calls the method prepare-sample-dir on the sample that is currently iterating
